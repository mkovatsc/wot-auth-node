...
model.generateToken = function(type, req, callback) {
  // var dn ={dn:JSON.stringify(req.connection.getPeerCertificate().subject)};
  rawDer = req.connection.getPeerCertificate().raw;
  var x5tsha256 = {['x5t#s256']: hash.sha256().update(rawDer).digest('base64url')};
  console.log(x5tsha256['x5t#s256']);
  var token;
  var secret;
  var user = req.user;
  var exp = new Date();
...
model.getUserFromClient = function(clientId, clientSecret, callback) {
    len = clientIdtoUser.length;
    for(i=0; i < len; i++ ){
      if(clientId === clientIdtoUser[i].clientId){
        callback(false, users[i]);
      }
    }
    callback(false,false);
};
